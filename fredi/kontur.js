  ...
  function showKonturScreen() {
    ...
    c.innerHTML =
      '<div class="kt-wrap">' +
        ...
        '<button class="kt-btn" onclick="window.showDeloGame&&window.showDeloGame()"><span class="em">💼</span>Своё дело <span style="font-size:.72rem;color:#34d399">🆕 симулятор</span><small>Бизнес-симулятор с Фреди для РФ-реалий: выбери дело, продумай стратегию и веди его 12 месяцев, решая проблемы. Дойди до 1,5 млн, не обанкротившись. Зашит урок золотой лихорадки: копать золото или продавать лопаты?</small></button>' +
        '<button class="kt-btn" onclick="window.showDvaPotokaGame&&window.showDvaPotokaGame()"><span class="em">🎙️</span>Два потока<small>Голосовой тренажёр двух уровней мышления: беглость и пересказ + порождение на ходу. 4 режима (эхо, экспромт, сказка-конструктор, двойная нагрузка), голос Фреди, очки потока и уровни сложности.</small></button>' +
        '<button class="kt-btn" onclick="window.showMnemoGame&&window.showMnemoGame()"><span class="em">🧠</span>Мнемо — тренажёр памяти<small>Освой реальные мнемотехники: дворец памяти, цепочка-история, образы цифр + чанкинг, метод ассоциаций. Выучи → примени способ → вспомни → честная оценка. С очками и уровнями сложности.</small></button>' +
        '<button class="kt-btn" onclick="window.showChuvstvaGame&&window.showChuvstvaGame()"><span class="em">\ud83d\udca0</span>Чувства <span style="font-size:.72rem;color:#34d399">\ud83c\udd95 эмоции</span><small>Тренажёр точного называния эмоций: ситуация + сигналы тела \u2192 что это за чувство? Стыд или вина, тревога или страх, зависть или ревность. Точно названное чувство теряет силу.</small></button>' +
        '<button class="kt-btn" onclick="window.showMyslGame&&window.showMyslGame()"><span class="em">\ud83d\udd0d</span>Мысль под допросом <span style="font-size:.72rem;color:#34d399">\ud83c\udd95 КПТ</span><small>Тренажёр по Беку: поймай автоматическую мысль, назови искажение (катастрофизация, чтение мыслей\u2026) и переформулируй точнее. Фреди оценит твою версию.</small></button>' +
        '<button class="kt-btn" onclick="window.showSkazhiNetGame&&window.showSkazhiNetGame()"><span class="em">\ud83d\uded1</span>Скажи \u00abнет\u00bb <span style="font-size:.72rem;color:#34d399">\ud83c\udd95 границы</span><small>Диалоговый тренажёр отказа: продавец, начальник, родня давят виной, срочностью и лестью \u2014 удержи вежливое \u00abнет\u00bb. Разбор от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showSobesGame&&window.showSobesGame()"><span class="em">\ud83d\udcbc</span>Собеседование <span style="font-size:.72rem;color:#34d399">\ud83c\udd95 работа</span><small>Фреди играет работодателя: вы называете сферу и должность, он собеседует. Три режима \u2014 мягкий, средний и жёсткий. Разбор ответов в конце.</small></button>' +
        '<button class="kt-btn" onclick="window.showPodrostokGame&&window.showPodrostokGame()"><span class="em">\ud83d\udeaa</span>Разговор с подростком <span style="font-size:.72rem;color:#34d399">\ud83c\udd95 родителям</span><small>Фреди играет вашего подростка. Шкала контакта двигается сразу: видно, какая реплика открыла разговор, а какая захлопнула створку.</small></button>' +
        '<button class="kt-btn" onclick="window.showOporaGame&&window.showOporaGame()"><span class="em">🧱</span>Опора <span style="font-size:.72rem;color:#818cf8">🆕 внутренний критик</span><small>Диалоговый тренажёр к курсу «Самооценка и уверенность»: внутренний критик давит после ошибки, похвалы, чужого успеха — сверхобобщением, кривой бухгалтерией, чтением мыслей. Отвечай не «я молодец» (та же шкала), а фактами, тоном друга и следующим шагом. Разбор от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showRolGame&&window.showRolGame()"><span class="em">🎭</span>Смени роль <span style="font-size:.72rem;color:#34d399">🆕 идентичность</span><small>Роль определяет поведение, а поведение определяет роль. Выйди из залипшей роли (Удобный, Невидимка, Шут, Атлант, Страдалец), меняя поступки по одному — шкала роли едет к новой. Разбор от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showSpiralGame&&window.showSpiralGame()"><span class="em">🌀</span>Спираль <span style="font-size:.72rem;color:#34d399">🆕 симулятор дня</span><small>Тренажёр к курсу «Снова живой»: выбери роль (уставший родитель, выгоревший, один после потери, молодой в тупике) и проживи её день по выборам. Живое действие или прозябание — шкала-«ядро» светлеет или гаснет. Урок на практике: действие раньше настроения разворачивает спираль вверх.</small></button>' +
        '<button class="kt-btn" onclick="window.showKlinGame&&window.showKlinGame()"><span class="em">🪓</span>Клин клином <span style="font-size:.72rem;color:#34d399">🆕 вовлечённость</span><small>Внимание — один канал. Из захватившей петли (скролл, руминация, тупик) выходят не волей и не в пустоту, а вкинувшись в другую вовлечённость — «клин»: дёшево войти + вектор к своей цели + тяга. Отличай верный клин от трёх ложных ходов. Аватар — стоячая волна: жив, пока течёшь. Разбор от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showParusGame&&window.showParusGame()"><span class="em">⛵</span>Парус <span style="font-size:.72rem;color:#34d399">🆕 тигр и торт</span><small>Тренажёр бизнес-мышления по курсу «Парус»: не строить с нуля, а вставать в то, что уже движется. Семь рейсов: увидеть поток и костыли → точка простоя → посчитать руками (маржа, безубыточность) → три условия → пара «тигр и торт» → ставки с бюджетом и туманом → свой парус под задачу. В финале — карточка паруса от Фреди с первым тестом и стоп-условием.</small></button>' +
        '<button class="kt-btn" onclick="window.showSignalGame&&window.showSignalGame()"><span class="em">📡</span>Сигнал <span style="font-size:.72rem;color:#2dd4bf">🆕 метод Шаталова</span><small>Тренажёр опорных сигналов: научись сжимать любой материал в опору из 5–7 элементов, которую невозможно забыть, — и разворачивать её обратно в знание. 6 уровней: отличи опору от простыни → сожми сам (Фреди оценит и покажет образец) → полный цикл на своём материале. Так запоминают учебник за вечер.</small></button>' +
        '<button class="kt-btn" onclick="window.showPerehodGame&&window.showPerehodGame()"><span class="em">🎚️</span>Переход <span style="font-size:.72rem;color:#5eead4">🆕 чьё это решение</span><small>Звонок из банка с «выгодными условиями». «Осталась одна, все берут». Мама про то, что все нормальные дети звонят каждый день. Ночное письмо коллеге, которое хочется отправить прямо сейчас. Не тест, а месяц: десять решений со второго по двадцать девятое число, и последствия переходят из сцены в сцену. У вас 60 000 свободных, силы, доверие близких и три паузы на весь месяц. К концу месяца сил мало — и вдумчивые ходы вроде «посчитать» и «уточнить» становятся недоступны, ровно как в жизни. Две сцены из десяти честные: отказ «на всякий случай» стоит там денег или возможности, так что игра наказывает и наивность, и паранойю. После выбора — рентген реплики: где именно вас двигали. В конце — карта месяца, точность различения и счёт по четырём областям: деньги, влияние, отношения, состояние. Спутник курса «Переход»: пройдите до курса и после, разница считается в цифрах.</small></button>' +
        '<button class="kt-btn" onclick="window.showLazejkaGame&&window.showLazejkaGame()"><span class="em">🕳️</span>Лазейка <span style="font-size:.72rem;color:#5eead4">🆕 буква и дух</span><small>Подросток обещал сесть за уроки — сел и смотрит в стену. Мелкий шрифт договора разрешил банку всё. Правило и то, что оно на самом деле разрешает, — разные вещи, и это игра про умение видеть разницу. «Гонка»: найдите ход, автор подтянет формулировку — ищите снова. «Разведка»: правило спрятано, нащупайте его пробами. «Заплатка»: правило пишете вы, а Фреди его обходит. «Своё правило»: принесите то, под которым живёте. Тренирует чтение договоров и регламентов, умение ставить правила детям и себе — и замечать те, что давно потеряли смысл.</small></button>' +
        '<button class="kt-btn" onclick="window.showAvtopilotGame&&window.showAvtopilotGame()"><span class="em">🛫</span>Автопилот <span style="font-size:.72rem;color:#5eead4">🆕 навык на триггер</span><small>Хотите чего-то, а этого нет — и рука тянется к кредиту или к «ну и ладно». Не вышло — и всё бросается. Тренажёр ставит на такой момент готовую мысль: индексация ресурсов и три пути, «а как ещё», «что я себе сказал», «чьё это решение». Как в курсе «Стимульный контроль» ставят поведение ребёнку: сначала поведение в чистом виде на сценах Фреди, потом привязка к триггеру среди сцен без него, три контекста, подкрепление и журнал на 14 дней — до автопилота.</small></button>' +
        '<button class="kt-btn" onclick="window.showChainikGame&&window.showChainikGame()"><span class="em">🫖</span>Чайник Рассела <span style="font-size:.72rem;color:#a78bfa">🆕 ясность мышления</span><small>Тренажёр интеллектуальной честности по Бертрану Расселу: на ком бремя доказательства, какие допущения протаскивают мимо тебя и сколько веры честно выдать утверждению. 6 уровней: чей чайник → скрытые допущения → весы веры → декалог Рассела → дуэль-разбор с Фреди → экзамен на собственном убеждении.</small></button>' +
        '<button class="kt-btn" onclick="window.showVsluhGame&&window.showVsluhGame()"><span class="em">💭</span>Мысль вслух <span style="font-size:.72rem;color:#818cf8">🆕 наладка мышления</span><small>Думать — приватное поведение: его видит только сам человек, поэтому его никто не поправил. Здесь мышление выносится наружу: Фреди думает вслух в разных контекстах (наука, новость, письмо, спор), ты повторяешь операции — своими словами, вопрос, главное, — он оценивает и дорабатывает. Отдельный акт «Попугай или понял?» чистит старое, заученное без обработки знание. Финал — экзамен со званием. Сначала навык, потом привычка думать.</small></button>' +
        '<button class="kt-btn" onclick="window.showIstoriaGame&&window.showIstoriaGame()"><span class="em">📖</span>Другая история <span style="font-size:.72rem;color:#fbbf24">🆕 изменение личной истории</span><small>Симулятор изменения личной истории (НЛП) на науке о реконсолидации памяти: прошлое как факт не изменить, но его след — можно. Фреди понимает, где ты «застреваешь» сейчас, находит раннюю сцену и недостающий ресурс — и ты проживаешь её иначе, а потом протягиваешь новый опыт к сегодняшнему дню. 5 актов, с бережным предохранителем.</small></button>' +
        '<button class="kt-btn" onclick="window.showOdiScreen&&window.showOdiScreen()"><span class="em">🧠</span>ОДИ: игра всерьёз <span style="font-size:.72rem;color:#a78bfa">🆕 для компании</span><small>Оргдеятельностная игра по методу Г.П. Щедровицкого — машина коллективного мышления для сложных тем. Создай игру, отправь ссылку компании (2–12 человек, без регистрации) — Фреди-игротехник проведёт по этапам: самоопределение → версии → проблематизация (атака на слабые идеи) → проект → рефлексия → итоговый протокол.</small></button>' +
        '<button class="kt-btn" onclick="window.showLgenijGame&&window.showLgenijGame()"><span class="em">🧲</span>Ленивый гений <span style="font-size:.72rem;color:#34d399">🆕 путь из 9 уровней</span><small>Двигатель гения: хотеть в 10 раз больше — делать в 10 раз меньше. Три акта: научись отличать дерзкие цели и ленивые рычаги от пустышек (быстрые задачи с мгновенной проверкой), обыграй Фреди в дуэлях за лучший ход, затем примени всё к своей реальной цели — с первым ходом на 2 минуты и чек-ином. Финал — экзамен на звание «Ленивый гений».</small></button>' +
        '<button class="kt-btn" onclick="window.showAlfavitGame&&window.showAlfavitGame()"><span class="em">🔤</span>Алфавит · новый код НЛП <span style="font-size:.72rem;color:#34d399">🆕 состояние</span><small>Классика Гриндера: произносишь букву вслух и одновременно жмёшь кнопку по пометке л/п/о. Речь и руки заняты разными задачами — внутренней болтовне негде развернуться. Три прохода, сбился — сначала, не дольше пяти минут. Прерывает прокручивание и меняет состояние.</small></button>' +
        '<button class="kt-btn" onclick="window.showFokusGame&&window.showFokusGame()"><span class="em">🎯</span>Фокус · N-back<small>Тренажёр рабочей памяти и концентрации. Реагируй, когда символ совпадает с тем, что был N шагов назад. Три уровня (1/2/3-назад), очки фокуса.</small></button>' +
        '<button class="kt-btn" onclick="window.showSchetGame&&window.showSchetGame()"><span class="em">⚡</span>Устный счёт<small>Скорость и числовая беглость: примеры на время + приёмы быстрого счёта (разбивка, ×11, проценты). Рекорды по времени.</small></button>' +
        '<button class="kt-btn" onclick="window.showKalibrGame&&window.showKalibrGame()"><span class="em">🎲</span>Калибровка<small>Вероятностное мышление: отвечай и ставь уверенность в %. В конце — карта твоей калибровки и разбор сверхуверенности (главной ловушки ума).</small></button>' +
        '<button class="kt-btn" onclick="window.showOshibkaGame&&window.showOshibkaGame()"><span class="em">🧯</span>Лови ошибку<small>Критическое мышление: в доводе спрятана логическая ошибка или искажение — назови какое. Соломенное чучело, ложная дилемма, корреляция ≠ причина и др. Разбор от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showDanetkiGame&&window.showDanetkiGame()"><span class="em">🕵️</span>Данетки<small>Латеральное мышление: странная ситуация, а ты разгадываешь её вопросами «да/нет» к Фреди-оракулу. Можно спрашивать голосом.</small></button>' +
        '<button class="kt-btn" onclick="window.showFermiGame&&window.showFermiGame()"><span class="em">📐</span>Прикидка · Ферми<small>Оценивай неизвестное через декомпозицию: «сколько мячей влезет в автобус?». Балл — за близость порядка величины. Разбор хода от Фреди.</small></button>' +
        '<button class="kt-btn" onclick="window.showAdvokatGame&&window.showAdvokatGame()"><span class="em">⚖️</span>Адвокат дьявола<small>Гибкость мышления: защити заданную позицию, даже если сам не согласен (стилменинг). Фреди-судья оценивает силу аргумента. Можно вслух.</small></button>' +
        '<button class="kt-btn" onclick="window.showSovetGame&&window.showSovetGame()"><span class="em">🛰️</span>Земля в опасности <span style="font-size:.72rem;color:#34d399">🆕 интрига</span><small>Симулятор политической интриги: ты спикер фракции в Мировом Совете, 6 путей спасения Земли, у всех тайные цели. Читай скрытые мотивы, ври, заключай и предавай сделки, стравливай соперников. Побеждает не тот, чей путь победил, а кто набрал очки. Разбор от Фреди: дипломат ты или кидала.</small></button>' +
        '<button class="kt-btn" onclick="window.showDostigatorGame&&window.showDostigatorGame()"><span class="em">🧭</span>Достигатор: поймай поток <span style="font-size:.72rem;color:#34d399">🆕</span><small>Тренажёр достигаторства: доска — это жизнь, по ней текут потоки-возможности. Энергию (КЦ) тратишь, когда прёшь силой воли, и зарабатываешь, когда ловишь поток. Собирай цели попутно, а не напролом. Разбор от Фреди: сёрфер потоков или пахарь.</small></button>' +
        '<button class="kt-btn" onclick="window.showKorkaGame&&window.showKorkaGame()"><span class="em">👑</span>Короли и капуста <span style="font-size:.72rem;color:#34d399">🆕</span><small>Тренажёр нетворкинга и чтения людей: карьера в банановой республике Анчурия. Наверх поднимают не выслугой, а поддержкой. У каждого своя валюта мотивации (признание/выгода/сила/связи) — читай тип, оказывай услуги в его валюте, давай раньше, чем просишь. Разбор от Фреди: крантехник ты или служака.</small></button>' +
        '<button class="kt-btn" onclick="window.showMandatGame&&window.showMandatGame()"><span class="em">🏛️</span>Мандат: цена кресла <span style="font-size:.72rem;color:#34d399">🆕 интрига</span><small>Симулятор психологии власти: ты депутат, Фреди подкидывает ситуации — наказ бабушки против партийной линии, конверт за голосование, застройщик и сквер. Задаёшь вводные (город, партия, бэкграунд, зачем шёл), потом выбираешь в дилеммах — размен между Людьми, Вертикалью, Аппаратом, Ресурсом и Совестью. Разбор от Фреди: каким ты стал и какой ценой.</small></button>' +
        ...
        '<button class="kt-btn" onclick="window.showProgressiveGame&&window.showProgressiveGame()"><span class="em">🚀</span>Вариатика — Progressive<small>Игра в 7 уровней по переходу в сектор ЧВ: конвертация ценностей, краники, ЛНСВ, БДК, поддержание связей. От «новичка» до «архитектора связей».</small></button>' +
        '<button class="kt-btn" onclick="window.showIntensiveGame&&window.showIntensiveGame()"><span class="em">💎</span>Вариатика — Intensive<small>Перспективное мышление УБ-масти: 3 уровня «Хрустального шара» — анализ, предсказание, формирование событий через канон 9 кружков.</small></button>' +
        '<button class="kt-btn" onclick="window.showImperativeGame&&window.showImperativeGame()"><span class="em">🛡️</span>Императив<small>Высшие навыки СБ-масти: 9 уровней пути — от «замри» к «законодателю». Лидерство, доминирование, владение силой как ресурсом.</small></button>' +
        '<button class="kt-btn" onclick="window.showExponentaGame&&window.showExponentaGame()"><span class="em">📈</span>Экспонента<small>Высшие навыки ТФ-масти: 9 уровней — от «работай рядом с домом» к «создавай новый рынок». Бизнес как война за клиентов.</small></button>' +
        '<button class="kt-btn" onclick="window.showPatternGame&&window.showPatternGame()"><span class="em">🎯</span>Паттерн: Охота на невидимое<small>Тренажёр главного навыка ЧВ-масти: видение скрытых правил. Фреди играет агента с секретным правилом — ты вычисляешь его. Путь от ЧВ-8 к ЧВ-10.</small></button>' +
