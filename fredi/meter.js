...
(function () {
    ...
    // именно на второй вопрос и только правдой: без аккаунта разговор
    // помнится семь дней на этом устройстве (free_tier.py на бэкенде,
    // 25.09.2026), с аккаунтом — без срока, и ждать придётся до завтра.
    function _whatBreaks(data, kind) {
        ...
        // 25.09.2026: без аккаунта разговор помнится семь дней на этом
        // устройстве (free_tier.py на бэкенде). «Не вспомнит ни слова» —
        // больше неправда, и писать это значило бы отнимать у человека
        // единственную причину вернуться завтра.
        return 'Разговор хранится неделю на этом устройстве, завтра Фреди начнёт с того же места. С аккаунтом — без срока и с любого устройства.';
    }
        ...
        try {
            ...
            overlay.innerHTML =
                '<div class="meter-modal">' +
                    ...
                    '<div class="meter-text">' + (who ? _esc(who) + ', ' + lead.charAt(0).toLowerCase() + lead.slice(1) : lead) +
                        body +
                        (anon ? '<br><br>Без аккаунта разговор хранится неделю и только на этом устройстве.' : '') +
                    '</div>' +
    ...
    function showAccountDoor(source) {
        try {
            ...
            lines.push('<b>Память — неделя и одно устройство.</b> Смените телефон или почистите ' +
                       'браузер — и разговор пропал. С аккаунтом он ваш насовсем.');
            overlay.innerHTML =
                '<div class="meter-modal">' +
                    '<div class="meter-emoji">🔓</div>' +
