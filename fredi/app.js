...
async function apiCall(endpoint, options = {}) {
    ...
    // Retry-логика. По аналитике (50% ai_response_error) видно, что бэк
    // переодически возвращает 5xx или вообще не отвечает в окно ~1 минуты.
    // Один автоматический retry с экспонентой убирает большую часть таких
    // временных сбоев — успех с первой попытки + успех со второй ≈ 80-85%.
    // Для AI делаем 2 ретрая (важнее), для остальных POST — 1, для GET — 0
    // (GET'ы поллятся часто, ретрай только увеличит нагрузку).
    const maxRetries = isAIEndpoint ? 2 : (method !== 'GET' ? 1 : 0);

    const _attemptOnce = async () => {
        ...
        try {
            ...
            // 5xx — retryable. Кидаем специальный класс ошибок, чтобы
            // ретраить только их (4xx — клиентская ошибка, не ретраим).
            if (!response.ok) {
                let detail = '';
                try { detail = (await response.json())?.error || ''; } catch {}
                const err = new Error(detail || `HTTP ${response.status}`);
                err._http_status = response.status;
                err._retryable = response.status >= 500 && response.status !== 501;
                throw err;
            }
            ...
            if (error.name === 'AbortError') {
                const e = new Error('Сервер не отвечает');
                e._retryable = true;
                throw e;
            }
            if (error._retryable === undefined && (!error._http_status)) {
                // Network failure (fetch threw) — ретраим
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                if (attempt > 0) {
                    ...
                    // Подсказка юзеру через ai-busy badge
                    try {
                        const t = document.getElementById('fredi-ai-busy-text');
                        if (t) t.textContent = isAIEndpoint
                            ? `Сервер перегружен, повторяю (${attempt}/${maxRetries})…`
                            : 'Повторяю запрос…';
                    } catch {}
                }
                ...
                lastErr = err;
                if (!err._retryable || attempt >= maxRetries) break;
                console.warn(`apiCall retry ${attempt + 1}/${maxRetries} for ${endpoint}:`, err.message);
            }
        }
        console.error(`API Error (after ${maxRetries + 1} attempts): ${endpoint}`, lastErr);
        // Превращаем ошибку в дружественную для юзера
        if (lastErr?._http_status >= 500) {
            throw new Error('Сервер перегружен. Попробуйте через минуту.');
        }
        if (lastErr?.message === 'Сервер не отвечает') {
            throw new Error('Сервер не отвечает. Попробуйте через минуту.');
        }
