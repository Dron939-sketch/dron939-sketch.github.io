...
function setupDashComposer() {
    ...
    async function send() {
        ...
        if (!text || busy) return;
        // Автовопрос из объявления или результата теста (openers.js) —
        // не сообщение человека: в счёт до карточки бесплатной версии
        ...
        // карточка приходила на третьем сообщении. 22.09.2026 владелец
        // сдвинул её: для всех — после ЧЕТВЁРТОГО сообщения, которое
        // человек набрал сам. Автовопрос из объявления и готовая фраза
 ...
 *
 * Подпись у строки — имя, возраст и город под размытием. Список персон
 * дал владелец 15.09.2026; они придуманные и ничьи: настоящие имена из
 * выгрузки на сайт не выносятся никогда, даже размытыми. Размытие — не
 * защита чьей-то тайны, а знак, что вопрос принесён человеком.
 * Персона подобрана под голос вопроса: «пытаюсь быть загадочной, а он
 * думает, что я обиделась» под мужским именем читается как ошибка, и
 * ошибкой бы и было. Мужские персоны стоят там, где вопрос звучит от
 * любого лица, — стыд, бессонница, срывы на близких, повышение.
 * Возраст и города — из реального распределения аудитории: медиана
 * 34 года, примерно семь женщин на трёх мужчин, города от Москвы
 * и Петербурга до Новокузнецка и Каширы.
 *
 * Тридцать вопросов про отношения добавлены владельцем 15.09.2026.
 * Каждый приведён к типографике сайта и сделан самостоятельным: в
 * оригинале это была цепочка («…Это конец? — А если он не прячется? —
 * И почему меня это задевает?»), а лента показывает строки вразбивку,
 * и «А если…» без предыдущей строки читается как обрывок чужой мысли.
 * Тема самая частая из всех: отношения, расставания и «уйти или
 * остаться» — 173 разговора из 1194 в годовой выгрузке.
 */
const DASH_QUESTIONS = [
    { q: 'Я представляю другого. Это уже измена?',                                     who: 'Марина, 34', city: 'Новокузнецк' },
    { q: 'Полгода без секса, и оба делаем вид, что так и надо',                        who: 'Олег, 55', city: 'Тюмень' },
    { q: 'Иногда я радуюсь чужим неудачам. Что со мной?',                              who: 'Дмитрий, 35', city: 'Новосибирск' },
    { q: 'В три часа ночи я точно знаю, что всё зря. Утром — нет',                     who: 'Анна, 32', city: 'Томск' },
    { q: 'Постоянное мысленное возвращение в ситуацию стыда',                          who: 'Сергей, 38', city: 'Пермь' },
    { q: 'Я не скучаю по маме. Со мной что-то не так?',                                who: 'Тимур, 34', city: 'Ярославль' },
    { q: 'Я срываюсь только на тех, кто точно не уйдёт',                               who: 'Артур, 29', city: 'Самара' },
    { q: 'Получил повышение — и впервые захотел уволиться',                            who: 'Иван, 45', city: 'Челябинск' },
    { q: 'Мне легче с чужими, чем с близкими',                                         who: 'Михаил, 52', city: 'Уфа' },
    { q: 'Я жду, когда он оступится, чтобы наконец уйти',                              who: 'Ирина, 48', city: 'Брянск' },

    { q: 'Муж «чинит машину» в гараже по три часа. Это конец или он просто прячется?', who: 'Мария, 31', city: 'Нижний Новгород' },
    { q: 'Он не прячется в гараже — он просто не хочет меня видеть?',                  who: 'Виктория, 42', city: 'Краснодар' },
    { q: 'Почему меня это задевает, если мы и так не разговариваем неделями?',         who: 'Елена, 36', city: 'Хабаровск' },
    { q: 'Мы не ссоримся, но я смотрю на него и ничего не чувствую. Быт съел любовь?', who: 'Диана, 37', city: 'Рязань' },
    { q: 'Любовь просто превратилась в привычку — сериалы под пиццу, и всё?',          who: 'Алия, 27', city: 'Воронеж' },
    { q: 'Почему он так странно смотрит на мою самоуверенную подругу?',                who: 'Карина, 25', city: 'Барнаул' },
    { q: 'Почему я вдруг начала проверять его телефон, хотя раньше было всё равно?',   who: 'Зарина, 23', city: 'Ростов-на-Дону' },
    { q: 'Соврала по мелочи — а теперь он проверяет мой. Это шпионы или недоверие?',   who: 'Амина, 22', city: 'Казань' },
    { q: 'А если он уже ушёл мысленно и просто забыл собрать вещи?',                   who: 'Полина, 20', city: 'Владивосток' },
    { q: 'Он собрал вещи и ушёл со словами «я запутался». Ждать или выкинуть щётку?',  who: 'Эмили, 24', city: 'Санкт-Петербург' },
    { q: 'Пытаюсь флиртовать, чтобы вернуть искру, а он видит во мне своего пацана',   who: 'Алиса, 21', city: 'Тольятти' },
    { q: 'Пытаюсь быть загадочной, а он думает, что я обиделась или туплю',            who: 'Мия, 19', city: 'Волгоград' },
    { q: 'Я всё делаю правильно, но он всё равно молчит неделю после ссоры',           who: 'София, 24', city: 'Астрахань' },
    { q: 'Если я напишу первой — я проиграю или спасу эти отношения?',                 who: 'Хлоя, 26', city: 'Омск' },
    { q: 'А если напишу, а он ответит, что ему просто нужно время?',                   who: 'Люба, 34', city: 'Саратов' },
    { q: 'Почему меня до сих пор тянет к тем, кто даже не читает мои сообщения?',      who: 'Соня, 24', city: 'Ижевск' },
    { q: 'Бывший написал «привет» в два часа ночи. Отвечать или это ловушка?',         who: 'Евгения, 31', city: 'Екатеринбург' },
    { q: 'А если это не ловушка — а я сама хочу в неё попасть?',                       who: 'Милана, 26', city: 'Иркутск' },
    { q: 'Каждую пятницу накрывает тоска по бывшему, хотя я сама его бросила',         who: 'Рита, 29', city: 'Кемерово' },
    { q: 'Муж ушёл к другой полгода назад, а теперь звонит и ноет. Зачем?',            who: 'Валентина, 47', city: 'Оренбург' },
    { q: 'Он говорит, что любит, но не уходит от жены. Ждать или бежать?',             who: 'Элина, 33', city: 'Махачкала' },
    { q: 'Встречаться с кем-то из страха остаться одной на выходные — это выход?',     who: 'Зоя, 38', city: 'Коломна' },
    { q: 'Встречаюсь с заботливым, а тянет к тому, кто меня игнорирует. Я больная?',   who: 'Аня, 26', city: 'Москва' },
    { q: 'Разум говорит «он хороший парень», а интуиция орёт «беги». Кого слушать?',   who: 'Инна, 35', city: 'Кашира' },
    { q: 'Я саботирую хорошие отношения, потому что думаю: я этого не заслуживаю',     who: 'Ксения, 30', city: 'Тула' },
    { q: 'Знаю все психологические трюки, но всё равно пишу первой после игнора',      who: 'Жанна, 28', city: 'Калуга' },
    { q: 'Устала быть идеальной. Если покажу характер, он сбежит или проснётся?',      who: 'Маргарита, 40', city: 'Иваново' },
    { q: 'Начала говорить «нет» и заниматься собой — и меня обвинили в эгоизме',       who: 'Лариса, 44', city: 'Смоленск' },
    { q: 'Мы вместе семь лет, и он больше не вызывает мурашек. Это конец?',            who: 'Яна, 32', city: 'Липецк' },
    { q: 'Конец — это когда он говорит, что любит, но третий день не выносит мусор?',  who: 'Эльвира, 36', city: 'Пенза' }
];


// От первого лица и на «вы» — как говорит сам Фреди на этом экране.
// Прежние строки описывали роли со стороны («Психолог выслушает,
...
const MODE_DESCS = {
    ...
    psychologist: 'Выслушаю, поддержу и мягко разберу, что с вами происходит',
    trainer: 'Дам конкретные задания и буду держать вас в графике'
};
...
function showPremiumLockPopup(modeName, opts) {
    ...
    const text = o.text || (isMode
        ...
        : 'Открывается с подпиской вместе с голосом, всеми режимами и памятью Фреди о каждом разговоре. Три дня полного Premium — 69 ₽, дальше 690 ₽ в месяц, отключается в один клик.');
...
function renderDashboard() {
    ...
    getUserStatus().then(status => {
        ...
        if (status.has_profile && status.profile_code) {
            ...
            apiCall(`/api/get-profile/${CONFIG.USER_ID}`).then(data => {
                ...
                if (archetype) {
                    if (codeEl) {
                        // Эмодзи архетипа вернулся по просьбе владельца
                        // 15.09.2026: без него строка «Прагматичный
                        // наставник / ваш архетип» выглядит служебной
    ...
    (function dashLiveChat() {
        ...
        const row = (item) => {
            ...
            // Сначала подпись, потом вопрос (решение владельца 15.09.2026):
            // слева — кто спрашивает, справа — что спросил.
            if (item.who) {
                const who = document.createElement('span');
                who.className = 'dash-live-who';
                who.textContent = item.who;
                // Город — отдельным элементом: на узком экране он прячется,
                // иначе подпись отъедает у вопроса треть строки и тот
                // разваливается на четыре строки вместо двух.
                if (item.city) {
                    const city = document.createElement('span');
                    city.className = 'dash-live-city';
        ...
        // Порядок перемешивается на каждую загрузку. Вопросов сорок, а за
        // визит человек видит от четырёх до десятка: при фиксированном
        // порядке три четверти списка не показались бы никому никогда,
        // и каждый заходящий читал бы одно и то же.
        const pool = DASH_QUESTIONS.slice();
        for (let j = pool.length - 1; j > 0; j--) {
            const k = Math.floor(Math.random() * (j + 1));
            [pool[j], pool[k]] = [pool[k], pool[j]];
        }

        // Человеку, который не хочет движения, показываем список без анимации.
        // Не весь: сорок строк — это экран прокрутки вместо подсказки.
        if (calm) {
            pool.slice(0, 12).forEach(q => list.appendChild(row(q)));
        ...
        for (; i < 3; i++) list.appendChild(row(pool[i]));
...
// Кнопка «Попробовать 3 дня — 69 ₽» в блоке подписки на результатах
