  ...
  function attachMic(btnId, taId) {
    ...
    btn.onclick = function () {
      ...
      if (_dkRec) {
        ...
        _dkRec = false; btn.classList.remove('rec'); btn.textContent = '🎤';
        return;
      }
      try {
        vm.sttOnly = true;
        ...
        vm.startRecording();
        _dkRec = true; btn.classList.add('rec'); btn.textContent = '⏹';
      } catch (e) { _dkRec = false; btn.classList.remove('rec'); btn.textContent = '🎤'; toast('Микрофон недоступен', 'error'); }
    };
  }
