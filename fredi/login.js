...
(function () {
    ...
    function _handleAuthError(data, mode, status) {
        ...
        if (err === 'email_exists') {
            // Авто-переключение register → login + префилл email + тёплый
            // тост. Раньше показывали красный «переключитесь на вкладку Вход» —
            // лишний клик и frustration. В БД 6 фейлов email_exists = юзер
            // случайно пошёл на «Регистрация» вместо «Вход».
            var em = '';
            try {
                var input = document.getElementById('faEmail');
                if (input) em = input.value.trim();
            } catch (e) {}
            _track('auth_email_exists_autoswitch', { email_known: !!em });
            _toast('У вас уже есть аккаунт — введите свой пин-код', 'info');
            _open('login', { source: _lastSource, prefillEmail: em });
            return;
        }
