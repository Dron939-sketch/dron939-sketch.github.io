    ...
    function _buildHtml(mode) {
        ...
        if (isRegister) {
            ...
            if (_lastSource === 'meter_auth') {
                subtitle = 'Первый разговор закончился. Чтобы Фреди помнил его завтра и продолжил с того же места, нужен аккаунт: имя, почта и пин-код из 4 цифр. С аккаунтом — ещё 5 минут сегодня и 5 каждый день, дальше пробная неделя за 290 ₽ или подписка 990 ₽ в месяц.';
            } else if (fromAppStart && hasAnonData) {
                subtitle = 'Фреди теперь работает с аккаунтами: имя, почта и пин-код из 4 цифр. Данные с этого устройства привяжутся к аккаунту и будут доступны отовсюду. С аккаунтом — 5 минут разговора каждый день бесплатно, дальше пробная неделя за 290 ₽ или подписка 990 ₽ в месяц.';
            } else if (fromAppStart) {
                subtitle = 'Фреди знакомится по имени: имя, почта и пин-код из 4 цифр — это всё. С аккаунтом — 5 минут разговора каждый день бесплатно, чтобы понять, ваше ли это. Дальше пробная неделя за 290 ₽ или подписка 990 ₽ в месяц.';
            } else {
                subtitle = 'Email станет вашим логином. Придумайте пин-код из 4 цифр. С аккаунтом — 5 минут разговора каждый день бесплатно, дальше пробная неделя за 290 ₽ или 990 ₽ в месяц.';
            }
        } else {
