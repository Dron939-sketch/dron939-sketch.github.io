...
(function () {
    ...
    var API_BASE = window.API_BASE_URL || (window.CONFIG && window.CONFIG.API_BASE_URL) || '';
    ...
    function _buildHtml(mode) {
        ...
        var subtitle;
        if (isRegister) {
            // 05.09: стена аккаунта приходит в конце первого разговора
            // (аноним получает 10 минут), и текст говорит, зачем аккаунт:
            // чтобы Фреди помнил разговор завтра. Числа минут — из
            // модели счётчика (subscription_meter): 5 в день с аккаунтом.
            if (_lastSource === 'meter_auth') {
                subtitle = 'Первый разговор закончился. Чтобы Фреди помнил его завтра и продолжил с того же места, нужен аккаунт: имя, почта и пин-код из 4 цифр. С аккаунтом — ещё 5 минут сегодня и 5 каждый день, дальше пробные три дня за 69 ₽ или подписка 690 ₽ в месяц.';
            } else if (fromAppStart && hasAnonData) {
                subtitle = 'Фреди теперь работает с аккаунтами: имя, почта и пин-код из 4 цифр. Данные с этого устройства привяжутся к аккаунту и будут доступны отовсюду. С аккаунтом — 5 минут разговора каждый день бесплатно, дальше пробные три дня за 69 ₽ или подписка 690 ₽ в месяц.';
            } else if (fromAppStart) {
                subtitle = 'Фреди знакомится по имени: имя, почта и пин-код из 4 цифр — это всё. С аккаунтом — 5 минут разговора каждый день бесплатно, чтобы понять, ваше ли это. Дальше пробные три дня за 69 ₽ или подписка 690 ₽ в месяц.';
            } else {
                subtitle = 'Email станет вашим логином. Придумайте пин-код из 4 цифр. С аккаунтом — 5 минут разговора каждый день бесплатно, дальше пробные три дня за 69 ₽ или 690 ₽ в месяц.';
            }
        } else {
        ...
        // сразу: 10 минут бесплатно, дальше 690 ₽/мес — человек решает
    ...
    // Гейт на входе снят (решение владельца от 02.09.2026, пересмотр
    // правила 31.08). Неделя с обязательной регистрацией на старте дала
    // жёсткие цифры: 46 показов гейта → 1 регистрация; десктопные сессии
    // умирали за 3-14 секунд, средняя — 26 секунд против 4:19 на мобиле.
    // Теперь аноним говорит сразу — сервер даёт ему 3 минуты разговора
    // (анонимный лимит), и регистрацию просит стена метера на пике
    // контакта (block_reason='auth' → openRegister из meter.js): аккаунт
    // даёт 10 минут вместо 3, и просьба звучит как продолжение разговора,
    // а не как турникет. Здесь остаётся только счётчик визитов и «Вход»
    // для тех, кто в этом браузере уже регистрировался, — им модалка
    // экономит шаг, а не отнимает разговор.
    function _maybeShowOnLoad() {
        ...
        _incrementVisit();
        if (_safeGet(LS_LAST_EMAIL)) {
            // Уважать снуз: закрыл модалку — сутки не докучаем, в этой
            // сессии скипнул — до конца сессии не показываем.
            try {
                if (sessionStorage.getItem('fredi_auth_skipped')) return;
                var dis = parseInt(localStorage.getItem('fredi_auth_dismissed_at') || '0', 10);
                if (dis && (Date.now() - dis) < 24 * 60 * 60 * 1000) return;
            } catch (e) {}
            _track('auth_gate_shown', { mode: 'login' });
            _open('login', { source: 'app_start', mandatory: false });
        }
    }
