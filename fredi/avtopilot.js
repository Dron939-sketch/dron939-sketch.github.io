...
(function () {
  ...
  var SKILLS = {
    ...
    // ── добавлены 21.09: отобраны из четырёх семейств — метапознание
    // (калибровка, альтернативы), регуляция состояния (стоики, КПТ, HALT),
    // исполнительные (импульс, откладывание), социальные (критика, спор,
    // просьба, объяснение). Критерий отбора: частый триггер, который
    // человек узнаёт в моменте, и поведение на двадцать секунд с
    // проверяемыми шагами.
    tyanetsya: {
      name: 'Рука тянется — десять минут', em: '✋',
      trigger: 'рука уже тянется: к корзине в магазине, к сладкому, к сигарете, к ленте, к «ответить резко»',
      why: 'Импульс живёт секунды, и решение принимается раньше мысли. Ставим паузу и вопрос о состоянии.',
      steps: [
        'Стоп: убрать руку или закрыть вкладку и назвать вслух, что делаю',
        'Что я хочу на самом деле — какое состояние, а не какой предмет',
        'Отложить на десять минут: таймер, и чем занять эти десять минут',
        'Через десять минут решить заново — и назвать решение словами'
      ],
      bridge: 'рука тянется — стоп, состояние, десять минут, решение',
      example: 'Курсор на «оформить заказ» за кроссовки. Стоп, закрываю вкладку. Хочу не кроссовки, а ощущение, что день не зря. Десять минут: выхожу на балкон. Через десять минут: не покупаю, иду на пробежку в старых.',
      nonTrigger: 'ситуации, где покупка или действие запланированы заранее и обдуманы'
    },
    otkladyvayu: {
      name: 'Откладываю — что именно страшно', em: '🕰️',
      trigger: 'ловите себя на «сделаю потом», открываете телефон вместо дела',
      why: 'Откладывают не дело, а его неприятную часть. Ставим называние этой части и первые пять минут.',
      steps: [
        'Назвать, какой именно кусок дела неприятен — не дело целиком',
        'Что самое плохое случится, если этот кусок сделать плохо',
        'Уменьшить до пяти минут: что успею за пять минут, и таймер',
        'После пяти минут решить: продолжаю или назначаю время'
      ],
      bridge: 'откладываю — неприятный кусок, худшее, пять минут, решение',
      example: 'Третий день не пишу отчёт. Неприятен не отчёт, а первый абзац, где надо признать срыв срока. Худшее — начальник и так знает. Пять минут: пишу этот абзац. Продолжил, отчёт ушёл.',
      nonTrigger: 'ситуации, где дело действительно не срочное и отложено осознанно, с датой'
    },
    uveren: {
      name: 'Точно знаю — а откуда', em: '🎚️',
      trigger: 'ловите себя на «точно», «наверняка», «он специально», «так всегда»',
      why: 'Уверенность и правота — разные вещи. Ставим калибровку: процент, проверка, что увижу, если ошибаюсь.',
      steps: [
        'Назвать утверждение одним предложением, как ставку',
        'Оценить уверенность в процентах и вспомнить, откуда она',
        'Что я увижу, если ошибаюсь, — конкретный признак',
        'Одно действие, которое проверяет: спросить, посмотреть, подождать'
      ],
      bridge: 'точно знаю — ставка, проценты, признак ошибки, проверка',
      example: '«Она обиделась, точно». Ставка: она обиделась. Уверен на 80, потому что не ответила два часа. Если ошибаюсь, увижу обычный ответ вечером. Проверка: пишу «как день?», а не «ты обиделась?».',
      nonTrigger: 'ситуации, где факт проверен и вопрос закрыт, или где вы сомневаетесь и так'
    },
    avdrug: {
      name: 'А вдруг — сценарий и план', em: '🌧️',
      trigger: 'крутится «а вдруг»: заболею, уволят, не сдаст, не долетит',
      why: 'Тревога держится на сценарии без конца. Ставим доигрывание сценария до плана и дихотомию контроля.',
      steps: [
        'Назвать сценарий до конца: что именно случится, одним предложением',
        'Оценить, насколько это вероятно, и что уже говорит за и против',
        'Что я сделаю, если это случится, — план Б в одной фразе',
        'Что из этого в моей власти сегодня — и сделать это, остальное отпустить'
      ],
      bridge: 'а вдруг — сценарий, вероятность, план Б, что в моей власти',
      example: '«А вдруг сократят». Сценарий: в ноябре уведомление. Вероятность: отдел закрывают, шансы реальные. План Б: два месяца компенсации и три знакомых, кому написать. В моей власти сегодня: обновить резюме за час. Остальное не моё.',
      nonTrigger: 'ситуации, где событие уже произошло, или где просто чего-то хочется'
    },
    razdrazhen: {
      name: 'Раздражён — четыре проверки', em: '🧯',
      trigger: 'всё бесит, огрызаетесь, к вечеру любая мелочь взрывает',
      why: 'Половина раздражения — не про людей, а про тело. Ставим проверку четырёх причин до любого разговора.',
      steps: [
        'Назвать состояние словом: раздражён, устал, зол',
        'Четыре проверки: голоден, устал, одинок, зол на что-то конкретное — что из этого да',
        'Закрыть базовое первым: поесть, десять минут тишины, написать человеку',
        'Важные разговоры и решения отложить до утра — и сказать это вслух'
      ],
      bridge: 'раздражён — слово, четыре проверки, базовое, отложить',
      example: 'Огрызнулся на «что на ужин». Состояние: раздражён. Проверки: голоден да, устал да, одинок нет, зол на письмо от клиента. Ем, десять минут молча. Разговор про ремонт — завтра, так и говорю.',
      nonTrigger: 'ситуации, где вы спокойны и просто решаете задачу'
    },
    oshibsya: {
      name: 'Ошибся — факт, причина, процедура', em: '🔧',
      trigger: 'заметили свою ошибку: не то отправил, забыл, сломал, подвёл',
      why: 'На «ошибся» обычно стоит «я идиот» или оправдание. Ни то, ни другое не мешает повторить. Ставим разбор процедуры.',
      steps: [
        'Назвать факт без оценки себя: что именно сделано не так',
        'Какой шаг привёл к ошибке — процесс, а не характер',
        'Что изменю в процедуре в следующий раз — одно конкретное правило',
        'Сказать вслух тому, кого это касается, — факт и что сделаю'
      ],
      bridge: 'ошибся — факт, шаг, правило, сказать',
      example: 'Отправил смету не тому клиенту. Факт: письмо ушло на чужой адрес. Шаг: набирал адрес по первым буквам. Правило: адрес проверяю перед «отправить». Клиенту: «отправил по ошибке, прошу удалить, ваша смета в следующем письме».',
      nonTrigger: 'ситуации, где ошибся кто-то другой, или где просто не получилось с первой попытки'
    },
    kritikuyut: {
      name: 'Критикуют — что из этого верно', em: '🪞',
      trigger: 'вам делают замечание, пишут разбор, говорят «ты не так»',
      why: 'На критику стоит либо защита, либо обвал. Ставим отделение факта от тона и вопрос «что верно».',
      steps: [
        'Пауза: не отвечать первые три секунды, не оправдываться',
        'Отделить факт от тона: что именно сказано, без «как»',
        'Что из этого верно — хотя бы одно',
        'Спросить уточнение или поблагодарить, и решить, что меняю'
      ],
      bridge: 'критикуют — пауза, факт без тона, что верно, уточнить',
      example: '«Ты всегда затягиваешь». Пауза. Факт: два последних срока сдвинулись. Верно: второй сдвиг был по моей вине. «Про второй согласен. Про первый уточню: там правки пришли за день до срока». Меняю: буферный день.',
      nonTrigger: 'ситуации, где вас хвалят, просят или просто задают вопрос'
    },
    sporyu: {
      name: 'Спорю — что он видит', em: '⚖️',
      trigger: 'хочется переубедить, доказать, ловите себя на «нет, ты не понимаешь»',
      why: 'Спор — это два монолога. Ставим вопрос вместо утверждения и поиск того, чего не вижу я.',
      steps: [
        'Назвать, о чём именно спор, одним предложением',
        'Что он видит, чего не вижу я — хотя бы одну вещь',
        'Задать вопрос вместо утверждения и дослушать до точки',
        'Назвать, где согласен, и решить, важен ли остаток спора'
      ],
      bridge: 'спорю — о чём, что он видит, вопрос, где согласен',
      example: 'Спор с женой про садик. О чём: частный или районный. Что видит она: дорога сорок минут, я не вожу. Вопрос: «что для тебя главное в саду?» Согласен: дорога решает. Остаток не важен.',
      nonTrigger: 'ситуации, где вы обсуждаете спокойно и никого не переубеждаете'
    },
    poprosit: {
      name: 'Нужно попросить — одно предложение', em: '🙋',
      trigger: 'нужна помощь, а вы тянете, «сам справлюсь», «неудобно»',
      why: 'Просьба откладывается из-за размытости и страха отказа. Ставим формулировку и цену отказа.',
      steps: [
        'Что именно прошу — одно предложение с глаголом и сроком',
        'У кого: кто уже делал такое, кому это ничего не стоит',
        'Цена отказа для меня — обычно ноль: что изменится, если скажут нет',
        'Попросить сегодня — сказать или написать это предложение'
      ],
      bridge: 'попросить — одно предложение, у кого, цена отказа, сегодня',
      example: 'Не успеваю забрать ребёнка в четверг. Прошу: «заберёшь Машу в четверг в пять?» У кого: сосед, у него дочь в той же группе. Цена отказа: попрошу бабушку. Пишу соседу сейчас.',
      nonTrigger: 'ситуации, где вы уже попросили или где помощь не нужна'
    },
    neponyal: {
      name: 'Не понял — а как ещё объяснить', em: '🧩',
      trigger: 'объясняете, а человек не понял: ребёнок, ученик, коллега, родитель',
      why: 'На «не понял» стоит «повторить громче». Ставим проверку, что именно не понято, и пример вместо правила.',
      steps: [
        'Спросить, что именно непонятно, — вопросом, а не повтором',
        'Дать пример из его жизни вместо правила',
        'Попросить повторить своими словами',
        'Если снова нет — второй способ: показать, нарисовать, сделать вместе'
      ],
      bridge: 'не понял — что именно, пример, своими словами, второй способ',
      example: 'Сын не понял дроби. Что именно: «половина» понятно, «три четверти» нет. Пример: пицца из четырёх кусков, съели три. Своими словами: «три куска из четырёх». Понял.',
      nonTrigger: 'ситуации, где человек понял и просто не согласен, или где не поняли вы'
    },
    custom: {
  ...
  // Каталог показывается группами: шестнадцать кнопок подряд не выбирают.
  var GROUPS = [
    { t: 'Желания и ресурсы', keys: ['hochu', 'tyanetsya', 'poprosit'] },
    { t: 'Неудачи и ошибки', keys: ['nevyshlo', 'oshibsya', 'otkladyvayu', 'mnogo'] },
    { t: 'Состояние', keys: ['zadelo', 'avdrug', 'razdrazhen'] },
    { t: 'Люди', keys: ['prosyat', 'kritikuyut', 'sporyu', 'neponyal'] },
    { t: 'Незнание и уверенность', keys: ['neznayu', 'uveren'] },
    { t: 'Своё', keys: ['custom'] }
  ];
  var ORDER = GROUPS.reduce(function (a, g) { return a.concat(g.keys); }, []);
  var CONTEXTS = [
  ...
  function scrSkill() {
    ...
    var list = GROUPS.map(function (g) {
      return '<div class="ap-note" style="margin:14px 0 4px;font-weight:600;color:#98a1b3">' + esc(g.t) + '</div>' + g.keys.map(function (k) {
        var s = SKILLS[k];
