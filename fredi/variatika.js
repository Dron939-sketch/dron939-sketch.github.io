...
(function () {
  ...
  function buildGen() {
    ...
    return 'Ты — генератор заданий для тренажёра «Вариатика» (чтение паттернов поведения).\n' +
      ...
      'ВАЖНО: каждую из четырёх меток (ИСТОРИЯ1, ИСТОРИЯ2, ЗАВЯЗКА3, РАЗВЯЗКА3) ставь С НОВОЙ СТРОКИ и пиши ровно так, с цифрой. ' +
  ...
  function parseRound(raw) {
    ...
    if (!out.s1 && hits.length && hits[0].key === 's2' && hits[0].start > 0) {
      out.s1 = text.slice(0, hits[0].start).trim();
    }
    return out;
  }
