...
(function () {
  ...
  function splitReaction(txt) {
    var t = clean(txt);
    // Границей служит само имя метки, а не перевод строки перед ней:
    // backend/services/ai_service.py перед возвратом делает
    // re.sub(r'\s+', ' ') и построчная разметка приезжает одной строкой.
    // С прежним `\n\s*СУДЬЯ` лукахед не срабатывал никогда, и вердикт
    // судьи целиком протекал в реплику слушателя.
    var lm = t.match(/СЛУШАТЕЛЬ\s*[:：]\s*([\s\S]*?)\s*(?:СУДЬЯ\s*[:：]|$)/i);
