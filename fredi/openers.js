...
(function () {
    ...
    //
    // Две фразы сравнивались 06–08.09: «а» — надёжные руки и «мы
    // справимся», «б» — качество жизни, психоэмоциональное состояние как
    // основа, регион и ситуация. Тест закончен, работает «б» (см.
    // _pickVariant). Проигравшая фраза оставлена рядом намеренно: она
    // записана в fredi/sounds/README.md как текст для озвучки, и вернуть
    // сравнение — это поменять одну строку, а не восстанавливать текст.
    // Цели в Метрике прежние: fredi_intro_<v> (показ) и
    // fredi_intro_reply_<v> (человек написал сам после ответа Фреди).
    var INTROS = {
    ...
    // Первая отправка за загрузку страницы — по ней решаем, показывать ли
    // представление тому, кто начал разговор сам.
    var _ownFirstDone = false;

    // A/B закончен 08.09.2026: остаётся вариант Б — выбор владельца по
    // смыслу фразы, а не по цифрам. Цифры выбрать не позволяли: A — 103
    // показа и 4 ответа, Б — 103 и 3, семь ответов на двоих, и одно
    // случайное сообщение переворачивало картину.
    //
    // Записанный вариант перезаписывается, а не читается: иначе тот, кому
    // в дни теста выпал А, остался бы на нём навсегда.
    var VARIANT = 'b';
        ...
        window.addEventListener('fredi:track', function (e) {
            ...
            if (ev === 'message_sent') {
                ...
                if (_autoAskPending) {
                    ...
                    _hideFollow();
                    _followShown = true;
                    // Представление звучало только тем, кого привёл
                    // автовопрос из объявления: _playIntro вызывался
                    // исключительно из _submitAsk. То есть человек,
                    // открывший Фреди сам и написавший своими словами, —
                    // как раз тот, кто ни разу здесь не был и не знает,
                    // кто ему отвечает, — не слышал ничего. Теперь
                    // представление идёт перед первым ответом в пустом
                    // чате, кто бы этот чат ни начал. Один раз за сессию —
                    // это обеспечивает _introDue.
                    // Проверять «чат пуст» здесь нельзя: событие приходит
                    // после того, как реплика человека встала в ленту.
                    // Признак другой — это первая отправка за загрузку
                    // страницы, а представления в этой сессии ещё не было.
                    if (!_ownFirstDone) {
                        _ownFirstDone = true;
                        var _introPending = true;
                        try { _introPending = !sessionStorage.getItem(INTRO_KEY); } catch (e) {}
                        if (_introPending) setTimeout(function () { _playIntro('own'); }, 900);
                    }
                    _onOwnMessage();
                }
                _onMessageSent();
