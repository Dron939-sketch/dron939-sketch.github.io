...
Модель синтеза задаётся НЕ здесь, а на сервере (Amvera):
    BLOG_TTS_FISH_MODEL = s2.1-pro-free

На 28.08.2026 API-баланс аккаунта Fish пуст: все платные модели отвечают
402, работает только s2.1-pro-free — без этой переменной весь пакет молча
уйдёт в Яндекс-фолбэк чужим голосом (--check это ловит). Глобальную
FISH_AUDIO_MODEL на бесплатную модель НЕ переводить: её условия разрешают
дообучение на запросах, статьи отдать не жалко, диалоги людей с
психологом — нельзя. BLOG_TTS_FISH_MODEL существует ровно для этого
разделения (Frederick #589).
