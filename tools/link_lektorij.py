...
RULES = [
    ...
    # «Переход» — флагман: курс плюс симулятор. Правило стоит высоко и
    # написано узко. Оно не про «как копить», а про «кто принял это решение
    # вместо вас»: границу знаний и торговлю чужим незнанием. Денежные
    # слова из правила убраны: «рассрочка» и «финансовая грамотность»
    # ловили статьи про умение отказывать и про архетипы денег — то
    # самое «примерно про то же», которое читается рекламой. Без этого
    # правила статья «Пролетарий или
    # интеллигенция: граница знаний» уходила на «Личные границы» — по слову
    # «границ», хотя речь о совершенно другой границе.
    (r"границ\w*[- ]знан|granica-znanij|пролетари|proletarij|интеллигенц|"
     r"незнани\w*[- ]как[- ]товар|кто[- ]реша\w*[- ]за[- ]вас|"
     r"классов\w*[- ]границ", "perehod"),
    ...
    (r"границ|granic|отказыва|otkazyva|сказать[- ]нет|ассертивн|удобн\w* (?:чело|для|быть)|быть удобн|контролирова|гиперконтрол",
     "lichnye-granicy"),
    ...
    (r"объясня|obyasnya|преподава|обучать взросл|передать опыт", "oratorskoe-iskusstvo"),
    (r"мышлен|myshlen|думать|dumat", "myshlenie"),
    (r"внимани|vnimani|фокус|fokus|концентрац|многозадачн", "kognitivistika"),

    # деньги и работа
    # «Дело жизни» — линия, накопление, «чем заниматься»: узко, чтобы не
    # перехватывать статьи про карьеру и бизнес у их курсов.
    (r"дело жизни|delo-zhizni|призван|prizvan|предназнач|prednazna|"
     r"найти себя|najti-sebya|чем (?:мне )?заняться|кем стать|kem-stat|"
     r"не знаю,? чего хочу|ne-znayu-chego-hochu", "delo-zhizni"),
...
# ——————————————————————————————————————————————————————————————————————
BLOCK_RE = re.compile(
    r"[ \t]*" + re.escape(MARK_OPEN) + r".*?" + re.escape(MARK_CLOSE)
    + r"\n?", re.S)
...
def strip(html):
    # Снимаем и перевод строки, который вставлялся вместе с блоком. Без
    # этого каждый прогон добавлял по пустой строке на статью, и отчёт
    # «изменилось бы 492 статьи» означал ровно это и ничего больше —
    # проверить по нему, что правки правил вправду что-то поменяли, было
    # невозможно.
...
def insert_tail(html, block):
    for pat in TAIL_ANCHORS:
        ...
        if m:
            # Отступаем к началу строки. Иначе блок встаёт после отступа
            # якоря, а сам отступ остаётся висеть отдельной строкой из
            # пробелов — и с каждым прогоном таких строк становится больше.
            start = html.rfind("\n", 0, m.start()) + 1
